...
from UsersManger import usersManger as usersManger
app = Flask(__name__)
...
#     param_userId=request.args.get('userId')
#     param_movieId=request.args.get('movieId')
#     param_movierating=request.args.get('rating')
#     if param_movieId!=None and param_movierating!=None and param_userId !=None:
#         movieManager().addMovieRatingToDb(int(param_movieId), int(param_userId), float(param_movierating))
...
def signup():
    param_userId=request.args.get('userId')
    param_username=request.args.get('name')
    param_useremail = request.get('email')
    param_password=request.get('password')
    if param_userId!=None and param_username!=None and param_useremail !=None and param_password!=None:
        usersManger.signUp(int(param_userId),str(param_username),str(param_useremail),str(param_password))

@app.route("/users/addfavourite", methods=['POST'])
def addfavourite():
    param_likedmovie=request.args.get('liked_movie_genre_ids')
    param_likedsongartists=request.args.get('liked_song_artists')
    param_bookauthor=request.args.get('liked_book_authors')
    param_likedsong=request.args.get('liked_song_genre')
    param_useremail = request.get('email')
    if param_likedmovie!=None and param_likedsongartists!=None and param_bookauthor!=None and param_likedsong!=None and param_useremail!=None:
        usersManger.addfavorites(param_likedmovie,param_likedsongartists,param_bookauthor,param_likedsong,param_useremail)