...
class MusicManager():
    ...
    def songlisteningtime(self,userId,songId,playing_times):
        con = self.__connectToDB()
        cur = con.cursor()
        userSongs = cur.execute('''SELECT song_id , playing_times FROM user_song_listening WHERE user_id = (?)''',
                                [userId]).fetchall()
        if songId[playing_times]=='':
            songId[playing_times]=1
        else:songId[playing_times]=songId[playing_times]+1
        con.close()
        return