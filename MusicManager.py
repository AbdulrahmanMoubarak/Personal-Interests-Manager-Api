...
class MusicManager():
    ...
    def addsonglistening(self, userId, songId):
        con = self.__connectToDB()
        cur = con.cursor()
        playing_times = cur.execute('''SELECT playing_times FROM user_song_listening WHERE user_id = (?) AND songId = 
        (?) ''', [userId], [songId], ).fetchall()
        if playing_times == '1':
            songId[playing_times] = 1
            cur.execute('''INSERT INTO user_song_listening values(?)''', [playing_times])
            con.close()
        else:
