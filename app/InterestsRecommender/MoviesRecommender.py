...
class MovieRecommender:
    ...
    def Contentbased(self, movieid, df, conn, numMovies):
        ...
        # 1246
        for chunk in pd.read_sql_query("""Select genres,movie_id from movies_metadata""", conn, chunksize=1000):
    ...
    def MatrixGenerator(self,conn, generate=False):
        try:
            if not generate:
                movie_similarity = pd.read_csv("app/similarity.csv").to_frame()
            else:
                df = pd.read_sql_query("""Select movie_id , title from movies_metadata""", conn)
                df['movie_id'] = df['movie_id'].astype(str)
            ...
            # apply pearson rule for standaralization
            movie_similarity = user_rating.corr(method='pearson')
            movie_similarity.to_csv("app/similarity.csv")
    ...
    def CollabBased(self, merged, conn):
        ...
        retList = similar_movie.sum().sort_values(ascending=False)
        return retList
    ...
    def __extractRecommendationFromContentDataFrame(self, resDf):
        dfDict = resDf.to_dict()
        mList = []
        for movie in dfDict:
